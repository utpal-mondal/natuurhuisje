...
function HostBookingsContent({ lang }: { lang: Locale }) {
  ...
  useEffect(() => {
    const loadBookings = async () => {
      try {
        ...
        // Fetch all landlord bookings (admin endpoint uses service role)
        const response = await fetch(
          `/api/admin/host-bookings?page=${currentPage}&itemsPerPage=${itemsPerPage}`,
        );
  ...
  const handleStatusUpdate = async (
    ...
    try {
      const response = await fetch("/api/admin/host-bookings", {
        method: "PATCH",
        headers: {
      ...
      {bookings.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-8 text-center">
          ...
          <p className="text-gray-500">
            {t?.bookings?.noBookingsHostMessage ||
                  ...
                  <div className="flex gap-6">
                    <div className="shrink-0">
                      <div className="relative w-24 h-24 rounded-lg overflow-hidden">
                        {booking.house.images &&
                        booking.house.images.length > 0 ? (
                          <Image
                            ...
                            <div className="space-y-1">
                              {(booking.regular_nights || 0) > 0 && (
                                <div className="flex justify-between text-gray-600">
                                  <span>
                                    {booking.regular_nights || 0}{" "}
                                    {(booking.regular_nights || 0) === 1
                                      ? "night"
                                      : "nights"}{" "}
                                    (regular)
                                  </span>
                                  <span>
                                    €{(booking.regular_price || 0).toFixed(2)}
                                  </span>
                                </div>
                              )}
                              {(booking.special_nights || 0) > 0 && (
                                <div className="flex justify-between text-gray-600">
                                  <span>
                                    {booking.special_nights || 0}{" "}
                                    {(booking.special_nights || 0) === 1
                                      ? "night"
                                      : "nights"}{" "}
                                    (special pricing)
                                  </span>
                                  <span>
                                    €{(booking.special_price || 0).toFixed(2)}
                                  </span>
                                </div>
                              )}
                              {booking.subtotal && (
